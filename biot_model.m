...
rho_1    = 1000 ;    % granite density
rho_0    = 1.2 ;   % air density
rho_s    = 9 ;      % overall density
sigma    = 12000 ;  % flow resistivity
F        = 0.99 ;   % porosity
alpha    = 1.005 ;  % tortuosity
nu       = 0.276 ;  % Poisson ratio
K_a      = 1.42e5 ; % bulk modulus of the fluid in the pores
K_s      = 1e8 ;    % bulk modulus of solid
Lambda   = 240e-6 ; % characteristic viscous dimension
Lambda_s = 480e-6 ; % characteristic thermal dimension
eta      = 1.82e-5 ;           % air dynamic viscosity 
N        = 86000*(1+0.05i) ; % shear modulus
gamma    = 1.4 ;             % adiabatic coefficient for air
c_p      = 1006 ;            % constant pressure
khi      = 25.9e-3 ;         % heat conductivity for air
...
omega    = 2*pi*freq ;
k0       = omega / c0 ;
k_t      = k0 * sin(theta) ; % lateral projection

Pr        = eta *c_p /khi ;  % Prandtl's number for air
M_s       = 8 *k_0_s /F /Lambda_s^2 ;
t_omega_s = rho_0 *Pr *omega *k_0_s /eta /F ;
beta1     = gamma - (gamma-1) .*(1 + 1 ./(-1i*t_omega_s).*...
            sqrt(1 - M_s/2 *1i.*t_omega_s)).^(-1) ;
K_b       = 2/3 *N *(1+nu) /(1-2*nu) ;
K_f       = K_a ./beta1 ;

% P, Q, R for material with incompressible skeleton
P         = 4/3 *N + K_b + (1-F)^2 /F *K_f ;
Q         = K_f*(1-F) ;
R         = F*K_f ;
% P         = ( (1-F)*(1-F-K_b/K_s)*K_s + F*K_s./K_f*K_b )./...
%             (1-F-K_b/K_s + F*K_s./K_f) + 4/3*N ;
% Q         = (1-F-K_b/K_s)*F*K_s ./ (1-F-K_b/K_s + F*K_s./K_f) ;
% R         = F^2*K_s./(1-F - K_b/K_s + F*K_s./K_f) ;

G         = sqrt(1-4i*alpha^2*eta*rho_0.*omega/sigma^2/...
            Lambda^2/F^2);
b         = -sigma./omega*F^2.*G ;

rho_a     = F*rho_0*(alpha - 1); % added density
rho_11    = rho_1 + rho_a - 1i*b ;
rho_12    = -rho_a + 1i*b ;
rho_22    = F*rho_0 + rho_a - 1i*b ;

Delta1    = (P.*rho_22 + R.*rho_11 - 2*Q.*rho_12).^2 - ...
          4*(P.*R-Q.^2).*(rho_11.*rho_22 - rho_12.^2) ;
delta_1   = sqrt(omega.^2/2./(P.*R-Q.^2).*...
          (P.*rho_22 + R.*rho_11 - 2*Q.*rho_12 - sqrt(Delta1))) ;
delta_2   = sqrt(omega.^2/2./(P.*R-Q.^2).*...
          (P.*rho_22 + R.*rho_11 - 2*Q.*rho_12 + sqrt(Delta1))) ;
...
alpha_1 = sqrt(delta_1.^2 - k_t.^2) ;
alpha_2 = sqrt(delta_2.^2 - k_t.^2) ;
alpha_3 = sqrt(delta_3.^2 - k_t.^2) ;

mu_1    = (P.*delta_1.^2 - omega.^2.*rho_11)./...
          (omega.^2.*rho_12 - Q.*delta_1.^2) ;
mu_2    = (P.*delta_2.^2 - omega.^2.*rho_11)./...
          (omega.^2.*rho_12 - Q.*delta_2.^2) ;
mu_3    = -rho_12./rho_22 ;
...
T_15 = 1i/N*omega./(beta.^2 + alpha_3.^2) .* ...
...
T_25 = - omega/N.*beta./(beta.^2 + alpha_3.^2).*(p_1.*(mu_2 - mu_3)./...
...
figure; plot(freq, abs(Refl_coeff));%.*mask) ;