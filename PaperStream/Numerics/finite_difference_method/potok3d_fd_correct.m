...
%% Mach_matrix
  ...
  for cur_node=1: num_nodes
    R_node(cur_node) = sqrt((node_coords(cur_node,1)-0.5).^2 + ...
        (node_coords(cur_node,3)-0.5).^2); 
  end
...
for cur_num= 1: num_nodes
 ...
 save('KD_matricies_m80','K','Dy')
%   load('KD_matricies','K','Dy')
  K = c^2*K/dx^2;
  Dy = 2*c*Dy/(2*dx);
...
for n= 1:size(node_coords,1)
   if( abs(node_coords(n,1) - 0.1) < 0.02 ) && ( abs(node_coords(n,2) - 0.5) < 0.02) && (abs(node_coords(n,3) - 0.5) < 0.02 ) 
        source_idx = n   
   end
end    
if source_idx == 0 
...
for n= 1:size(node_coords,1)
   if( abs(node_coords(n,1) - 0.5) < 0.02 ) && ( abs(node_coords(n,2) - 0.5) < 0.02 ) && (abs(node_coords(n,3) - 0.5) < 0.02 ) 
        reciver_idx_1 = n   
   end
end

for n= 1:size(node_coords,1)
   if( abs(node_coords(n,1) - 0.8) < 0.02 ) && ( abs(node_coords(n,2) - 0.5) < 0.02 )&& (abs(node_coords(n,3) - 0.5) < 0.02 )  
        reciver_idx_2 = n   
   end
end
...
mask = mask_1 .* mask_2 ;
 ...
 res_r1 = real(fft(res_r1_f.*mask));
 res_r2 = real(fft(res_r2_f.*mask));

figure;
plot(c*(1:n_t)*dt, res_r1)
hold all
plot(c*(1:n_t)*dt, res_r2)
